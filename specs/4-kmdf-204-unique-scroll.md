...
2. Unique identity stays: INF `MagicMouseDriver-kmdf-204-scroll.inf`, dest/ServiceBinary `MagicMouseDriver-kmdf-204-scroll.sys`, DriverVer bumped per build and derived from the `-Version` / `-DriverVerDate` defaults in `scripts/kmdf-204-scroll-build.ps1` (the build `Fail 3`s on any INF mismatch, `:72-76`). Fail if DriverVer is `08/30/2026,2.0.4.0` or `08/31/2026,2.0.4.0` (oem26 / PR #3 collision).
