...
Do not lose the local restore package.
...
behaviour. It survives only as the rollback target below.

## Restore (keep this)

Keep a known-good **16940C0F**-signed stage folder for every build that was ever installed. The
oldest one kept is the **2026-09-01 2.0.4.1** stage folder, retained as dated rollback evidence
rather than as the shipped version:
...
2.0.4.1 folder, so a bare run **is** the rollback; point `-Stage` at the `kmdf-204-sign-2043`
folder to reload the 2.0.4.3 package (the last one that exists as a binary; 2.0.4.6 has no stage
folder until it is built). The stage folders live under a queue root; set `MM_QUEUE_ROOT` (or pass
`-QueueRoot`) when yours is not the script default. Do not `Copy-Item` onto System32. Do not delete
oem16.
...
| Sign sys+cat (`-SkipPnputil`) | Pass — Authenticode **Valid** in the `community-dry` dry-run folder. That dry run predates the Inf2Cat requirement and catalogued with `New-FileCatalog`; its `.cat` must not be published or handed to `pnputil`. |
...
1. Confirm the `kmdf-204-sign` restore folder still has 16940C0F-signed sys+cat.
2. Re-catalogue that dry folder before anything touches `pnputil`: run `Setup-Community.ps1` **without** `-SkipPnputil` from the `community-dry` folder. A full run rebuilds the `.cat` with `Inf2Cat.exe`, re-signs it, and only then does `pnputil /add-driver <unique INF> /install`. Do **not** hand the `New-FileCatalog` `.cat` left by the earlier `-SkipPnputil` dry run to `pnputil` — that catalog is not installable.
...
Out of this ship. hidclass sees a mouse with wheel. PTP needs a later virtual HID, not a larger SDP overlay (0x50). Tray SendInput is a fake.
