...
`Driver.c:371`, in the source of the live 2.0.4.3 build:
...
buffer for its 142-byte `reqCtx->Scratch`, set `BufferSize = MM_ACL_MAX_PARSE`, ORed
...
PE section headers of the four builds parked in the 2.0.4.3 build folder, parsed from the files:
...
`C:\Windows\INF\setupapi.dev.log`, sections 18:13:50.039 and 01:10:51.156 — byte-identical
`pnputil /add-driver … /install` commands naming the same staging directory. That directory is
overwritten in place, which is how one command line delivered two different binaries.
...
Both sides of the comparison have now been measured on hardware.

**PRE-fix, on the 01:44 binary.** Window 23:36:18-23:37:05, 15 active seconds:
...
arrived normally. A second pre-fix window, `Rid12Count` +64 over 5 active seconds, also produced
0. An earlier window was correctly voided rather than reported as a negative (`Rid12Count` +0 —
nobody was touching the mouse).

**POST-fix, on the restored 17:45 `08E91E37` binary** (per-finger rule, no `refId`):

| measured | value |
| --- | --- |
| `Rid12Count` | **+5714** over 84 active seconds |
| wheel events delivered | **24** (sum +2640 → 22 notches at 120/notch) |
| hwheel events delivered | **8** (sum −720) |
| attribution | all 32 events from the 0323 `Col01` instance |

The decoder was proven from the same byte offset rather than assumed: a `usButtonFlags` histogram
over the same capture shows `0x0001` ×7 and `0x0002` ×7 — real left/right clicks — alongside
`0x0400` ×24 (wheel) and `0x0800` ×8 (hwheel). A misread offset could not produce a coherent
button histogram and a coherent wheel histogram simultaneously.

#### How strong the pre-fix zero is — strong, not absolute

All 32 post-fix events landed in a **single 1.34 s burst at t = 57.19-58.53 s**, not spread across
the 84 seconds. So the honest post-fix rate is **0.286 events per active second**, and the pre-fix
zero must be judged against that rate, not against the burst rate:

| basis | probability of observing 0 events pre-fix if the driver were healthy |
| --- | --- |
| time (15 active seconds at 0.286 events/s) | **p ≈ 0.003** |
| per report (694 reports) | **p ≈ 0.04** |

That is strong evidence, not proof. The pre-fix zero is consistent with the reference-finger rule
killing emission, and it is not consistent with a healthy driver under the same hand motion — but
it is a p-value, and it is quoted here as one. The burst structure is also a caution about the
post-fix number: 84 active seconds of hand motion produced one 1.34 s window of notches, so
"scroll works" post-fix means "scroll can be made to emit", not "emits continuously".

### Host model (both rules, replayed)
...
1. Staged the last binary observed reading a real percentage —
   `MagicMouseDriver-kmdf-2.0.4-scroll-08E91E37.sys`, 25600 bytes,
   sha256 `08e91e37af3b…cababd6ac` (**note:** an earlier hand-off quoted this prefix wrong in two
   nibble groups) — into its own restore stage folder, with `STAGE-MANIFEST.txt`, `RESTORE.ps1`
   and `RESTORE-README.md`. The existing `kmdf-204-sign*` stages were left untouched.
2. The first run aborted in step B on a **bug in `RESTORE.ps1` itself**: its `Run` helper did
...
3. Second run: Inf2Cat → `signtool sign` + `verify /pa` on `.sys` and `.cat` (both
...
4. The script reported "battery not confirmed" because it read the tray log immediately, while the
