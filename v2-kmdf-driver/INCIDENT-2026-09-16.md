...
| scroll regression | fix implemented on `fix/scroll-refid-stationary`: delete the `refId` pre-pass (`:82-111`) and the two `if (id == refId)` wrappers (`:168`, `:173`), and raise the detent to 16 (`GestureEngine.h:42`) so sensitivity halves without any gesture going dead. Max-delta and per-report-cap alternatives both measure 15 notches on skewed-speed drags vs 7. **Never compiled** — same missing WDK compiler payload |
| **#41** — no host gate covers multi-finger emission with a stationary low-id contact | filed; the gap is closed by the two new checks on that branch |
