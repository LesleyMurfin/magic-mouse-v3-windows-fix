...
Tray SELECT for 0323 should pull **`MagicMouseDriver-kmdf-2.0.4-scroll-<sha8>.sys`** (FileVersion 2.0.4.3; INF dest `MagicMouseDriver-kmdf-204-scroll.sys`). Leave **`MagicMouseDriver-kmdf-apr30-pointer-AD5D244B.sys`** as the pointer-only baseline (live name `MagicMouseDriver.sys`). Refuse **`MagicMouseDriver-kmdf-may20-pointerdead-559B136A.sys`**. PATH-A stays in `v1-binary-patch/`.
